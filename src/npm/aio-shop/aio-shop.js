...
export default class AIOSHOP {
    constructor(props) {
        ...
        this.importHTML = undefined;
        this.getShippingOptions = undefined;
        this.getDiscounts = undefined;
        this.getExtras = undefined;
        this.cartCache = undefined;
        this.checkDiscountCode = undefined;
        ...
        this.setCartCount = (obj) => {
            let { product, variantId, count } = obj;
            if (count === 0) {
                ...
                if (!cartItem) {
                    newCart = this.cart.concat(obj);
                }
                else {
                    newCart = this.cart.map((o) => o.product.id === product.id && o.variantId === variantId ? Object.assign(Object.assign({}, o), { count,time:new Date().getTime() }) : o);
                }
                this.cart = newCart;
            }
            if (this.cartCache) {
        ...
        this.renderList = ({ products, before, after, addToCart, popup }) => {
            if (popup) {
        ...
        this.renderProductSlider = ({ items, before, after, label, onShowAll }) => {
        ...
        this.renderProductCard = ({ product, variantId, html, imageSize, type, floatHtml, addToCart,onClick }) => {
            // let message = AIOValidation({
            //     validations:[
            //         ['required','',{title:'AIOShop.renderProductCard(obj) --- obj.product',value:product}],
            //         ['=',['horizontal' , 'vertical' , 'shipping'],{title:'AIOShop.renderProductCard(obj) --- obj.type',value:type}]
            //     ]
            // })
            ...
            return (<ProductCard actions={this.actions} getState={this.getState.bind(this)} key={product.id + ' ' + variantId} product={product} variantId={variantId} {...{ html, imageSize, type, floatHtml, addToCart }} onClick={onClick}/>);
        };
...
function List(props) {
    ...
    function update(searchValue) {
        ...
        if (!searchValue) {
            ...
            newProducts = products.filter((product) => isMatch(searchValue.split(' '), product.name));
        }
